...
import { 
    ...
    Info, Play, Pause, Activity, Wifi, ArrowLeftRight,
...
const ScheduleTimeline = ({ 
  ...
  const [warningTooltip, setWarningTooltip] = useState(false); 
  const scrollContainerRef = useRef(null);

  // --- 1. Extract Diagnostics & Dynamic Duration ---
  const diagnostics = result?.schedule_diagnostics || {};
  const hasOverflow = diagnostics.had_overflow || false;
  
  // Convert frame_ticks (ms) to seconds for the ruler display
  // Default to 10s if missing
  const TOTAL_DURATION_SEC = (diagnostics.frame_ticks || 10000) / 1000;
    ...
    result.active_links.forEach(link => {
       ...
       if (!groups[key]) {
           groups[key] = { 
               ...
               airtimeFwd: 0,
               airtimeRev: 0,
               // Mark this row if it is in the overflow list
               isOverflowing: diagnostics.overflow_links?.some(
                   ([s, t]) => (s === n1Id && t === n2Id) || (s === n2Id && t === n1Id)
               )
           };
       }
  ...
  const handleHeaderClick = (e) => {
      // Prevent closing when clicking interactive elements in header
      if (e.target.closest('button') || e.target.closest('.interactive-icon')) return;
      if (isFullScreen) {
    ...
    >
        ...
        >
            <div className="flex items-center gap-3 font-bold text-xs uppercase tracking-wider select-none">
                ...
                <span>TDMA Schedule</span>
                
                <span className={`font-mono font-normal ${darkMode?'text-blue-400':'text-blue-600'}`}>
                    {(currentTime * TOTAL_DURATION_SEC * 1000).toFixed(0)}ms 
                    <span className="opacity-50 mx-1">/</span> 
                    {(TOTAL_DURATION_SEC * 1000).toFixed(0)}ms
                </span>

                {/* --- HEADER WARNING ICON --- */}
                {/* Kept minimal since App.jsx has the main warning, but useful for tooltip details */}
                {hasOverflow && (
                    <div 
                        className="interactive-icon flex items-center gap-1 text-red-500 bg-red-100 dark:bg-red-900/30 px-2 py-0.5 rounded animate-pulse cursor-help relative"
                        onMouseEnter={() => setWarningTooltip(true)}
                        onMouseLeave={() => setWarningTooltip(false)}
                    >
                        <AlertTriangle size={14} />
                        
                        {/* Detailed Warning Tooltip (Unique to Timeline) */}
                        {warningTooltip && (
                            <div className={`absolute top-full left-0 mt-2 p-3 w-64 rounded-lg shadow-xl border z-50 pointer-events-none ${darkMode ? 'bg-slate-800 border-red-500/50 text-gray-200' : 'bg-white border-red-200 text-gray-800'}`}>
                                <div className="font-bold text-red-500 mb-2 flex items-center gap-2 border-b border-red-500/20 pb-1">
                                    <AlertCircle size={14} /> Schedule Overflow
                                </div>
                                <div className="text-[11px] space-y-2">
                                    <div className="flex justify-between font-mono bg-red-500/10 p-1 rounded">
                                        <span>Frame Stretch:</span>
                                        <span className="font-bold">{(diagnostics.frame_stretch || 1.0).toFixed(2)}x</span>
                                    </div>
                                    <div className="mt-2">
                                        <div className="opacity-70 mb-1">Impacted Links:</div>
                                        <ul className="list-disc pl-4 space-y-0.5 text-red-400 font-mono">
                                            {diagnostics.overflow_links?.slice(0, 5).map(([s, t], i) => (
                                                <li key={i}>{s} ↔ {t}</li>
                                            ))}
                                            {(diagnostics.overflow_links?.length || 0) > 5 && (
                                                <li>...and {diagnostics.overflow_links.length - 5} more</li>
                                            )}
                                        </ul>
                                    </div>
                                </div>
            ...
            <div className="flex items-center gap-4">
                {isActive && (
                    <div className="flex items-center gap-4 text-[10px] font-medium opacity-70 mr-4">
                        <div className="flex items-center gap-1"><div className="w-3 h-3 bg-blue-500 rounded-sm"></div> FWD</div>
                        <div className="flex items-center gap-1"><div className="w-3 h-3 bg-purple-500 rounded-sm"></div> REV</div>
                    </div>
                )}
                <button 
                    onClick={() => {
            ...
            <div className="flex min-w-full min-h-full">
                ...
                <div className={`sticky left-0 z-30 w-44 flex-shrink-0 border-r ${headerBorder}`} style={{ backgroundColor: stickyBg }}>
                    ...
                    {rows.map(row => {
                        ...
                        // Color label red if overflowing
                        const labelClass = row.isOverflowing ? 'text-red-500 font-bold' : '';
                        ...
                        return (
                            ...
                            >
                                <div 
                                    className={`font-bold text-xs truncate flex items-center gap-1 cursor-pointer w-full ${labelClass}`}
                                    onMouseEnter={(e) => {
                                        const rect = e.currentTarget.getBoundingClientRect();
                                        setTooltipData({
                                            x: rect.right + 10,
                                ...
                                >
                                    ...
                                    {row.isOverflowing && <AlertTriangle size={10} className="text-red-500 flex-shrink-0" />}
                                    <Info size={10} className="opacity-50 ml-auto" />
                                </div>
                                <div className="opacity-50 text-[9px] mt-0.5 flex justify-between">
                                    <span>{(row.distance).toFixed(0)}m</span>
                                    <span>{(row.capacitySum / 1000).toFixed(1)}k</span>
                                </div>
                            </div>
                        );
                    })}
                ...
                <div className="flex-1 relative min-w-[600px]"> 
                    ...
                    <div className={`h-6 border-b sticky top-0 z-10 select-none ${headerBorder}`} style={{ backgroundColor: stickyBg }}>
                        {Array.from({ length: Math.ceil(TOTAL_DURATION_SEC) + 1 }).map((_, i) => (
                    ...
                    <div className="relative" onClick={handleTimelineClick}>
                        {rows.map(row => {
                            const isHovered = row.id === hoveredLinkKey;
                            // Highlight background if overflowing
                            const rowBg = row.isOverflowing && !isHovered
                            ...
                            return (
                                <div 
                                    key={row.id} 
                                    className={`h-12 border-b relative w-full group ${gridBorder} ${rowBg} ${isHovered ? (darkMode ? 'bg-cyan-900/30' : 'bg-cyan-50') : (darkMode ? 'hover:bg-gray-800/30' : 'hover:bg-blue-50/30')}`}
                                    onMouseEnter={() => setHoveredLinkKey(row.id)}
                                    onMouseLeave={() => setHoveredLinkKey(null)}
                                >
                                    {/* Forward Fragments */}
                                    ...
                                    {/* Vertical Grid Lines */}
                                    {Array.from({ length: Math.ceil(TOTAL_DURATION_SEC) }).map((_, i) => i > 0 && (
        ...
        {tooltipData && (
            <div 
                ...
                style={{ 
                    left: tooltipData.x, 
                    top: Math.min(window.innerHeight - 200, Math.max(10, tooltipData.y - 50)) 
                }}
            >
